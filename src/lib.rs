...
impl Session {
    ...
    pub fn consume(&mut self, nonce: chacha20::Nonce, data: &[u8], descriptors: &[Descriptor])
        ...
        for d in descriptors.iter() {
            match *d {
                ...
                Descriptor::Static => {
                    ...
                    self.rs = Some(PubKey(make_fixed!(u8, 32, try!(NoiseBody::read(&data[off..off+DH_LEN+MAC_LEN])
                                                                   .decrypt(&k, &nonce, &a[..])))));
                    off += DH_LEN + MAC_LEN;
                },
                Descriptor::DHSS => dh_branch!(s, rs),
...
impl NoiseBody {
    pub fn read(buf: &[u8]) -> NoiseBody {
        ...
        for i in 0..MAC_LEN { mac[i] = buf[buf.len() - MAC_LEN + i] }
        NoiseBody {
            ciphertext: buf[..buf.len() - MAC_LEN].to_vec(),
            mac: poly1305::Tag(mac)
        }
    }
...
impl BoxN {
    ...
    pub fn unbox(receiver: KeyPair, ct: &[u8]) -> Result<Vec<u8>, ()> {
        ...
        s.consume(chacha20::Nonce([0u8; 8]), ct, &[Descriptor::Ephemeral, Descriptor::DHES]).map(|(_,p)| p)
    }
}
